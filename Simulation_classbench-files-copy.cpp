...
const vector<int> fieldOrder = {2, 4, 0, 3, 1};
...
void printRules () {
	...
	for (int i=0; i < ruleCount; i++) {
		...
		for (int j=0; j < MAX_DIMENSIONS; j++) {
			cout << "("<< rules[i]->getDimRangeMin(j) << ", " << rules[i]->getDimRangeMax(j) << ") ";
		}
		cout << endl;
	}
}
...
int main (int argc, char* argv[]) {
	...
	if (argc < 5)
	{
		...
		cout << "./simulation {Rules_File} {Packet_File} {maximum_nodes_per_leaf} {Brute = 0,1}";
		exit(-1);
	}
    //numRules = stoi(argv[1]); //numRules is a global variable.
	//int numPackets = stoi(argv[2]);
    srand(time(NULL));
	//rules = readFilterFileClassBench(argv[1]);
	...
	maxRulesPerNode = std::stoi(argv[3]);
	brute_force_choice = std::stoi(argv[4]);
	//packets = GeneratePacketsFromRuleset(rules, numPackets);
	packets = generatePacketsFromFile(argv[2]);
	int numPackets = packets.size();
	...
	//vector<int> fieldOrder = {0, 1, 2, 3, 4};
	...
	for (int j = 0; j < 10; j++) {
		...
		for (int i = 0; i < numPackets; i++) {
			//classifyFirst (mainRoot, packets[i], dim, fieldOrder, ruleClassified);
	...
	std::ofstream outfilePacketTimes ("PacketTimes.txt");
	
    if (!outfilePacketTimes) {
        std::cerr << "Failed to open the packet times file for writing.\n";
        exit (0);
    }
	// Write the vector's contents to the file
    for (const auto& value : times) {
        outfilePacketTimes << value << '\n';  // Writes each element on a new line
    }

    // Close the file (optional here, since the file will close when outfile goes out of scope)
    outfilePacketTimes.close();
	
	// // Open a file for writing
	// std::ofstream outfileBroadest ("BroadestClassification.txt");
    ...
    // }
	// // Write the vector's contents to the file
    // for (const auto& value : queryResultBroadest) {
    //     outfileBroadest << value << '\n';  // Writes each element on a new line
    // }

    // // Close the file (optional here, since the file will close when outfile goes out of scope)
    // outfileBroadest.close();
	...
	if (brute_force_choice){
		...
		elapsed = end - start;
		cout << "\n The time taken to brute force classify " << numPackets << " packets is:" << elapsed.count() << " seconds\n";
